...
Future<List<ListPreview>> ListV(id) async {
  final response = await http.get(Uri.parse('http://localhost:5000/json/$id.json'));

  if (response.statusCode == 200) {
    List ListVV = json.decode(response.body);
    return ListVV.map((json) => ListPreview.fromJson(json)).toList();
  } else {
...
class OilShip extends StatefulWidget {
  ...
  const OilShip({Key? key, required this.id}) : super(key: key);
...
class _OilShipState extends State<OilShip> {
  ...
  void initState() {
    super.initState();
    futureListPreview = ListV(widget.id);
  }
